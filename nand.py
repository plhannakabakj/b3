...
bot = telebot.TeleBot('7374201904:AAFQfvMAzgWefrnXaX8WmGWe1y9nJTIr9Go')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi3_cooldown and (datetime.datetime.now() - bgmi3_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown ❌. Please Wait 10sec Before Running The /bgmi3 Command Again."
                bot.reply_to(message, response)
                return
...
💥 /bgmi3 : Method For bgmi3 Servers. 
